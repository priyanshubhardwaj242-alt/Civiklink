...
export default function Schemes() {
  ...
  const filtered = useMemo(() => {
    ...
    if (q.trim()) {
      ...
      list = list.filter((s) => s.name.toLowerCase().includes(t) || s.desc.toLowerCase().includes(t) || s.tag.toLowerCase().includes(t));
    }
    ...
    return list;
  }, [q, ministry, sector, beneficiary, route, sort, catFilter]);
    ...
    <div className="max-w-[1400px] mx-auto px-6 py-8 text-slate-100">
      <div className="rounded-2xl border border-slate-800 bg-[#0f1e37] p-6 md:p-8">
        ...
        <div className="mt-5 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
          {[
            { label: 'FILTER BY MINISTRY', value: ministry, set: setMinistry, options: ['All Ministries', ...MINISTRIES] },
            { label: 'SECTOR & INDUSTRY', value: sector, set: setSector, options: ['All Sectors', ...SECTORS] },
            { label: 'ASSISTANCE / FINANCIAL TYPE', value: assistance, set: setAssistance, options: ['All Assistance Types', 'Loan', 'Subsidy', 'Grant', 'Insurance', 'Pension'] },
            { label: 'FILTER BY BENEFICIARY', value: beneficiary, set: setBeneficiary, options: ['All Beneficiary Groups', ...BENEFICIARIES] },
            { label: 'FILTER BY APPLICATION ROUTE', value: route, set: setRoute, options: ['All Application Routes', ...APPLICATION_ROUTES] },
          ].map((f) => (
            <div key={f.label}>
              <div className="text-[10px] font-semibold text-slate-500 tracking-widest mb-1">{f.label}</div>
              <select value={f.value} onChange={(e) => f.set(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-orange-500">
        ...
        <div className="mt-5 flex items-center justify-between text-sm text-slate-400">
          <div>Showing <span className="text-white font-semibold">{filtered.length}</span> of {SCHEMES.length} Verified Schemes</div>
          <div className="flex items-center gap-2">
            ...
            <select value={sort} onChange={(e) => setSort(e.target.value)} className="bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5 text-sm text-slate-200 focus:outline-none focus:border-orange-500">
              {['Most Relevant', 'Name (A to Z)', 'Name (Z to A)', 'Highest Loan Limit', 'Lowest Interest Rate', 'Recently Updated'].map((o) => (<option key={o}>{o}</option>))}
            </select>
          </div>
        </div>
      </div>
          ...
          <Link to={`/schemes/${s.id}`} key={s.id} className="group rounded-2xl border border-slate-800 bg-[#0f1e37] p-5 hover:border-orange-500/50 hover:bg-[#122549] transition-colors">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
              <div className="rounded-lg bg-slate-950/60 border border-slate-800 p-2.5">
                <div className="text-slate-500">MAX SUPPORT / LOAN</div>
                <div className="text-orange-300 font-semibold mt-0.5">{s.loan}</div>
              </div>
              <div className="rounded-lg bg-slate-950/60 border border-slate-800 p-2.5">
                <div className="text-slate-500">INTEREST RATE</div>
                <div className="text-white font-semibold mt-0.5">{s.rate}</div>
              </div>
            </div>
