    ...
    <div className="text-slate-100">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-[1400px] mx-auto px-6 pt-16 pb-20 text-center">
          ...
          <p className="mt-6 text-slate-300 max-w-2xl mx-auto text-base md:text-lg">
            Empowering citizens, entrepreneurs, and farmers across India with rule-based eligibility checks, official Gazette guidelines, and direct portal routes.
          </p>
          <div className="mt-9 flex flex-col sm:flex-row justify-center gap-3">
            <Link to="/recommendations" className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl text-white font-semibold bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 shadow-lg shadow-orange-900/40 transition-colors">
              <Sparkles size={18} /> Find Matching Schemes <ArrowRight size={17} />
            </Link>
            <Link to="/schemes" className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl text-slate-100 font-semibold border border-slate-700 bg-slate-900/50 hover:bg-slate-800 transition-colors">
              <Search size={18} /> Explore All 90 Schemes
            </Link>
          </div>
        </div>
      ...
      <section className="max-w-[1400px] mx-auto px-6 mt-14">
        <div className="rounded-2xl border border-slate-800 bg-[#0f1e37] p-6">
          ...
          <div className="mt-4 flex flex-col md:flex-row gap-2">
            <div className="flex-1 flex items-center gap-2 px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 focus-within:border-orange-500">
              ...
              <input
                ...
                placeholder="Search e.g. MUDRA, women, Ayushman..."
          ...
          <div className="mt-4">
            <div className="text-xs text-slate-500 mb-2">Popular Focus Areas:</div>
            <div className="flex flex-wrap gap-2">
              {FOCUS_TAGS.map((t) => (
                <Link key={t} to={`/schemes?q=${encodeURIComponent(t)}`} className="text-xs px-3 py-1.5 rounded-full border border-slate-700 bg-slate-900/60 text-slate-200 hover:border-orange-400 hover:text-orange-300 transition-colors">{t}</Link>
              ))}
            </div>
          </div>
        </div>
          ...
          {CATEGORIES.map((c) => {
            ...
            return (
              <Link to={`/schemes?cat=${c.id}`} key={c.id} className="group rounded-2xl border border-slate-800 bg-[#0f1e37] p-5 hover:border-orange-500/40 hover:bg-[#122549] transition-colors">
                ...
                <p className="mt-2 text-sm text-slate-400 leading-relaxed">{c.description}</p>
                <div className="mt-4 flex items-center justify-between text-sm">
                  <span className="text-slate-500 text-xs">{c.tag}</span>
                  <span className="inline-flex items-center gap-1 text-orange-400 group-hover:text-orange-300">View Details <ChevronRight size={14} /></span>
                </div>
              </Link>
            );
          })}
      ...
      <section className="max-w-[1400px] mx-auto px-6 mt-16">
        <div className="rounded-3xl border border-slate-800 bg-[#0a1628] p-8 md:p-10">
          <div className="text-center">
            <div className="inline-flex items-center gap-2 text-xs px-3 py-1.5 rounded-full bg-sky-500/10 border border-sky-500/25 text-sky-300">Official Information Architecture</div>
            <h2 className="mt-4 text-2xl md:text-3xl font-bold text-white" style={{ fontFamily: 'Georgia, serif' }}>Directly Sourced from Official Government Gazettes &amp; Portfolios</h2>
            <p className="mt-3 text-slate-400 max-w-3xl mx-auto">CivicLink operates as an independent civic-tech portal providing deterministic eligibility guidance derived from published ministry guidelines, official portals, and authorized channel partner directories.</p>
          </div>
          <div className="mt-8 grid md:grid-cols-4 gap-4">
            {[
              ...
              <div key={i} className="rounded-2xl border border-slate-800 bg-[#0f1e37] p-5">
                ...
                <h4 className="mt-3 text-white font-semibold">{f.title}</h4>
      ...
      <section className="max-w-[1400px] mx-auto px-6 mt-16 mb-4">
        <div className="rounded-3xl bg-gradient-to-r from-[#12213c] to-[#0f1e37] border border-orange-500/20 p-10 text-center">
          <h2 className="text-2xl md:text-3xl font-bold text-white" style={{ fontFamily: 'Georgia, serif' }}>Ready to Discover Schemes You May Be Eligible For?</h2>
          <p className="mt-3 text-slate-400">Takes less than 2 minutes. Answer a few questions to get personalized government scheme guidance.</p>
